(function () {
  ...
  // The cat api is down, so now to add /dog just in case
  function getDog(parameters) {
    // var type = parameters[0]
    // if (!['gif', 'png', 'jpg'].includes(type)) {
    //     type = "jpg,png"
    // }
  ...
  function loadDragDrop() {
    ...
    dropZone.addEventListener('dragover', e => {
      ...
      document.getElementById('input').style = 'background-color: antiquewhite;'
    })
    ...
    dropZone.addEventListener('dragleave', e => {
      ...
      e.preventDefault()
      e.dataTransfer.dropEffect = 'copy'
      document.getElementById('input').style = 'background-color: #fbf2d9;'
    })
    ...
    dropZone.addEventListener('drop', e => {
      ...
      fetch('/upload/image', {
        ...
        .then(response => response.text())
        .then(text => {
          const imgurURL = text.match("var result = '(.*)';")[1]
          sendMessage(imgurURL)
        })
    })
  }
})()